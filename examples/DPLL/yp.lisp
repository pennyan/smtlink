...
(encapsulate()
  (Local (defthm lemma-1 ; needed for rationalp-of-B-sum
           (implies (and (integerp nco_lo) (integerp nco_hi)
                         (<= 0 nco_lo) (<= nco_lo nco_hi)
                         (hyp-fn (list :v0 v0 :dv dv :g1 g1 :Kt Kt))
                         (nco-list-ok (list nco_lo (- nco_hi)) g1 v0))
                    (rationalp (B-term nco_hi v0 dv g1 Kt)))))
  ...
  (define B_sum (nco_lo nco_hi v0 dv g1 Kt)
    ...
    :returns (x rationalp
                :hyp :guard
                ;; :hints (("Subgoal *1/4"
                ;;          :use ((:instance rationalp-of-B-term (nco nco_hi)))))
                )
