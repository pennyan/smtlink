...
V0=s.isReal("V0")
DV=s.isReal("DV")
hypothesis=s.ifx(s.equal(s.times(EXPT_GAMMA_MINUS_H,EXPT_GAMMA_H),1),s.ifx(s.lt(0,EXPT_GAMMA_MINUS_H),s.ifx(s.notx(s.lt(s.times(1,s.reciprocal(5)),EXPT_GAMMA_MINUS_H)),s.ifx(s.notx(s.lt(H,1)),s.ifx(s.notx(s.lt(640,H)),s.ifx(s.equal(G1,Q(1,3200)),s.ifx(s.notx(s.lt(V0,Q(9,10))),s.ifx(s.notx(s.lt(Q(11,10),V0)),s.equal(DV,0),False),False),False),False),False),False),False),False)
