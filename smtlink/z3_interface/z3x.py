...
class to_smt:
	...
	def get(self, x,y): return x>=y
	def set(self, x,y): return x<=y
...
# usage prove(claim) or prove(hypotheses, conclusion)
	def prove(self, hypotheses, conclusion=0):
		...
		if self.solver.check() == unsat: return self.status(True)  # It's a theorem
		else: return self.status(str(self.solver.model())) # provide a counter-example string
