...
class to_smt(object):  # mrg: added (Object) to make this a "new style" class
    class status:
	...
	cx = self.solver.model() if res == sat else None
	self.solver.pop()
    
        if res == unsat: print "proved"
        elif res == sat: print "counterexample: " + str(cx)
        else: print "failed to prove:  " + str(res)
	return res == unsat
